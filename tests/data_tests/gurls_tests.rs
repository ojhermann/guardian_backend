...
pub fn insert_get_and_delete_work() {
    ...
    // add two urls
    insert_works(database_pool::get(database_url_key), url_value);
    insert_works(database_pool::get(database_url_key), url_value);

    // get urls with url_value
    let id_values = get_works(database_pool::get(database_url_key), url_value, 2);

    // delete all urls with url_value
    id_values
        .iter()
        .for_each(|id_value| delete_works(database_pool::get(database_url_key), *id_value));

    // confirm there are no rows in gurl with the url_value
    get_works(database_pool::get(database_url_key), url_value, 0);
}
