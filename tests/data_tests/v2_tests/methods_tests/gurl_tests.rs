use guardian_backend::data::v2::database::database_pool::{self, DatabasePool};
use guardian_backend::data::v2::methods::gurl;
...
pub fn get_most_recently_added_gurl_works() {
    ...
    let girl_maybe = tested_fnc_query_result.unwrap();
    assert!(girl_maybe.is_some());
    let id_of_tested_query = girl_maybe.unwrap().id;
...
fn get_gurls_works() {
    ...
    let most_recently_added_gurl_maybe = most_recently_added_query_result.unwrap();
    let start_id = most_recently_added_gurl_maybe.unwrap().id;
    ...
    let most_recently_added_query_result = gurl::get_most_recently_added_gurl(&pooled_connection);
    let most_recently_added_gurl_maybe = most_recently_added_query_result.unwrap();
    let end_id = most_recently_added_gurl_maybe.unwrap().id + 1;
